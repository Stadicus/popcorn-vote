...
const reservedSlugs = new Set(['assets']);
...
for (const entry of await readdir(deployedOutput, { withFileTypes: true })) {
	if (!entry.isDirectory() || entry.name === 'assets' || slugs.has(entry.name)) continue;
	try {
...
if (checkOnly) {
	...
	console.log(`Checked ${locales.length} localized pages, the language gateway, and sitemap.xml.`);
} else {
