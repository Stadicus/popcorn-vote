...
const analyticsEvents = (currentLocale) => `\t<script>
			(() => {
				...
				// Refusing the star bar, by the close button or by Escape; the bar
				// raises this itself, because the delegate above only sees links and
				// binding the button alone would miss every Escape. Following the bar
				// is already counted as a github-click from the star-bar area, so the
				// two answers stay distinguishable.
				document.addEventListener('star-bar-refused', () => track('star-bar-dismiss'));
